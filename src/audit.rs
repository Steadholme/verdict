...
//!
//! Verdict emits two notable events: a `verdict.tuple.write` when a relation tuple is written, and
//! a `verdict.check.deny` when an `/api/check` resolves to `allowed=false`. Neither carries any
//! sensitive value — only the logical fields (actor / action / target / severity / detail).
...
mod tests {
    ...
    fn event_serializes_to_safe_fields() {
        ...
        assert_eq!(v["action"], "verdict.tuple.write");
        assert_eq!(v["source"], "verdict");
    }
